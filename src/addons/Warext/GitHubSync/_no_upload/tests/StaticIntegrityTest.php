...
if (!is_array($addon) || ($addon['version_string'] ?? '') !== '0.7.0 Alpha 7') $errors[] = 'addon.json version mismatch';
...
foreach ([
    ...
    'wgh_conflicts.html', 'wgh_conflict_view.html', 'wgh_actions.html', 'wgh_xfrm.html'
] as $template)
{
...
foreach ([
    ...
    'Entity/XfrmRelease.php',
    'Service/XFRM/Capability.php',
    'Service/XFRM/CredentialProvider.php',
    'Service/XFRM/ApiClient.php',
    'Service/XFRM/DownloadResolver.php',
    'Service/XFRM/ReleaseSynchronizer.php',
    'Admin/Controller/Traits/XfrmActions.php'
] as $file)
{
...
foreach ([
    ...
    'xf_wgh_sync_object', 'xf_wgh_delivery', 'xf_wgh_user_mapping', 'xf_wgh_conflict', 'xf_wgh_xfrm_release'
] as $table)
{
    if (!str_contains($setup, $table)) $errors[] = 'Setup missing table: ' . $table;
}
foreach (['upgrade5000050Step1', 'upgrade5000050Step2', 'upgrade7000070Step1'] as $upgrade)
{
...
if (!str_contains($dispatcher, 'ReleaseSynchronizer')) $errors[] = 'EventDispatcher does not use XFRM ReleaseSynchronizer';
...
foreach (['actionUserMappings', 'actionConflicts', 'actionConflictResolve', 'label_prefix_map', 'actionActions', 'actionWorkflowDispatch', 'outbound_field_map', 'inbound_field_map', 'actionXfrm', 'xfrm_resource_id', 'xfrm_api_key_ref'] as $feature)
{
